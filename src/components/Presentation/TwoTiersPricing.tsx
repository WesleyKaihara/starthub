import {
  chakra,
  Stack,
  VStack,
  HStack,
  Text,
  Container,
  Box,
  Icon,
  Button,
  Divider,
  SimpleGrid,
  useColorModeValue,
} from "@chakra-ui/react";
import { BiCheck } from "react-icons/bi";

const plansList = [
  {
    title: "Gratuito",
    subLabel: "Acesso limitado na plataforma",
    price: 0,
    features: [
      "Acesso limitado ferramentas",
      "Exposição de Startups",
      "Criação de discussões",
      "Interações ilimitadas",
      "Suporte via email",
    ],
  },
  {
    title: "Premium",
    subLabel: "Acesso completo a plataforma",
    price: 59.99,
    disabled: true,
    features: [
      "Funcionalidades do Plano Gratuito",
      "Acesso Completo as Ferramentas",
      "Oportunidades Exclusivas",
      "Mentorias",
      "Suporte via WhatsApp",
    ],
  },
];

const TwoTiersPricing = () => {
  return (
    <Container maxW="5xl" p="6">
      <chakra.h2 fontSize="5xl" fontWeight="bold" textAlign="center" mb={5}>
        Planos
      </chakra.h2>
      <SimpleGrid columns={{ base: 1, md: 2 }} spacing={2} mt={4}>
        {plansList.map((plan, index) => (
          <PricingCard key={index} {...plan} />
        ))}
      </SimpleGrid>
    </Container>
  );
};

interface PricingCardProps {
  title: string;
  subLabel: string;
  price: number;
  features: string[];
  disabled?: boolean;
}

const PricingCard = ({
  title,
  subLabel,
  price,
  features,
  disabled = false,
}: PricingCardProps) => {
  return (
    <Box
      minW={{ base: "xs", sm: "sm" }}
      border="1px solid"
      borderColor={useColorModeValue("gray.400", "gray.600")}
      rounded="md"
      marginInline="auto"
      my={3}
      _hover={{
        boxShadow: useColorModeValue(
          "0 4px 6px rgba(160, 174, 192, 0.6)",
          "0 4px 6px rgba(9, 17, 28, 0.4)"
        ),
      }}
    >
      <Box p={6}>
        <chakra.h2 fontSize="2xl" fontWeight="400">
          {title}
        </chakra.h2>
        <Text fontSize="sm">{subLabel}</Text>
        <Text fontSize="4xl" fontWeight="bold" my={4}>
          R${price.toFixed(2).toString().replace(".", ",")}
          <Text as="sub" fontSize="md" fontWeight="normal">
            /mês
          </Text>
        </Text>
        <Button
          bgGradient="linear(to-br, #735EF3, #998FF0)"
          color="white"
          _hover={{ bgGradient: "linear(to-br, #735EF3, #B5AEF2)" }}
          variant="solid"
          size="md"
          rounded="md"
          w="100%"
          isDisabled={disabled}
          _disabled={{
            bg: "gray.400",
            cursor: "not-allowed",
          }}
        >
          {disabled? "Em breve": "Adquirir"}
        </Button>
      </Box>
      <Divider />
      <VStack p={6} spacing={4} alignItems="flex-start">
        <Text fontSize="sm" fontWeight="semibold">
          O que está incluso
        </Text>
        {features.map((feature, index) => (
          <HStack key={index} spacing={3}>
            <Icon as={BiCheck} h={4} w={4} color="green.500" />
            <Text fontSize="sm" color="gray.500">
              {feature}
            </Text>
          </HStack>
        ))}
      </VStack>
    </Box>
  );
};

export default TwoTiersPricing;
