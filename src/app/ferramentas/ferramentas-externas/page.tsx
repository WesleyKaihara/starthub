...
export default function Page(): ReactNode {
  ...
  const [ferramentas, setFerramentas] = useState<Ferramentas[]>([]);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setLoading(true);
    try {
      const { data } = await AnalysisService.sugestoesFerramentas(
        selectedProject
      );
        ...
        >
          {![1, 3].includes(currentStep) && (
            <>
              <Heading as="h1" size="lg" my={4} textAlign={{ base: "center" }}>
                Serviço de Sugestões de Ferramentas
              </Heading>
              <Text color="gray.700" mb={6} textAlign={{ base: "justify" }}>
                Nosso serviço utiliza inteligência artificial para ajudar você a
                encontrar as melhores ferramentas para utilizar durante o
                desenvolvimento da sua startup. Siga os passos para selecionar
                seu projeto, descrever sua ideia e obter sugestões de
                ferramentas úteis e relevantes.
              </Text>
              <Divider mb={6} />
            </>
          )}
          ...
          {currentStep === 1 && (
            <Box textAlign="center">
              <Heading as="h2" size="lg" mb={4}>
                Bem-vindo ao nosso serviço de sugestões de ferramentas!
              </Heading>
          ...
          {currentStep === 2 && (
            <form onSubmit={handleSubmit} style={{ width: "100%" }}>
              <VStack spacing={4}>
                <FormControl>
                  <FormLabel>Selecione um projeto (opcional):</FormLabel>
                  <Select
          ...
          {currentStep === 3 && (
            <>
              ...
              {ferramentas.map((ferramenta, index) => (
                ...
                >
                  <Text>
                    <strong>Nome:</strong> {ferramenta.name}
                  </Text>
                  <Text>
                    <strong>Descrição:</strong> {ferramenta.description}
                  </Text>
                </Box>
              ))}
